...
1. **No registration at all.** Create a single Garage Key and the app derives a fresh robot avatar for every trade: that's all you need. Since no email, phone, username or any input from the user is needed, there is no possible way to make a mistake and doxx yourself. Your Robot avatars cannot be linked to you.
...
4. **One identity -> one trade, enforced.** RoboSats automatically rotates to a new robot identity for every single trade. A robot that completed an order cannot trade again. No other exchange has this feature and **it is critical for privacy!** There is no way for observers to know that several trades have been made by the same user if he used different robot avatars.
