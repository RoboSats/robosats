...
| `federationLottery.ts` | `federationLottery`                                      | Randomises coordinator order for federation neutrality    |
...
Returns coordinators in **randomised order** for federation neutrality. Called at runtime
...
- **`federationLottery` randomisation is the federation-neutrality mechanism** — it must
  always run before presenting coordinators to the user. Bypassing it (e.g., sorting by
  some preference) violates the neutrality product invariant.
...
- `federationLottery` must not be replaced with a deterministic sort — federation
  neutrality is a product invariant.
